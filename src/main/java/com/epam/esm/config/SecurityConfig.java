...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        ...
        http
            ...
            .formLogin().disable()
            .securityMatcher("/module/**")
            .authorizeHttpRequests(registry -> registry
                            .requestMatchers("/module/login").permitAll()
                            .requestMatchers("/module/signup").permitAll()
                            .requestMatchers("/module/refresh").permitAll()
                            .requestMatchers("/module/secured").permitAll()
                            .requestMatchers(GET, "/**")
                            .hasAnyAuthority(Role.GUEST.name(), Role.USER.name(), Role.ADMIN.name())
                            .requestMatchers(POST, "/module/orders")
                            .hasAnyAuthority(Role.USER.name(), Role.ADMIN.name())
                            .requestMatchers("/module/**").hasAuthority(Role.ADMIN.name())
                            .anyRequest()
                            .authenticated()
                );
        return http.build();
